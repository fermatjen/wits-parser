...
public class WikiLeftOverCase implements WITSCase {
    ...
    public String runCase() {
        ...
        int l4_loc = source.indexOf("{{", offset);
        int l5_loc = source.indexOf("}}", offset);

        int l6_loc = source.indexOf("\r\n||", offset);
        int l7_loc = source.indexOf("\r\n|", offset);

        int l8_loc = source.indexOf("\r\n<LB", offset);
        int l9_loc = source.indexOf("\\\\", offset);

        if(l1_loc != -1 || l4_loc != -1 ||
                l5_loc != -1 || l6_loc != -1 || l7_loc != -1 || l8_loc != -1 || l9_loc != -1){
                return "FAIL";
        }
